...
using WinAuthSample.AppServices;
...
public partial class AccountPage : Controller
{
    ...
    //winauth changes below:
    public ActionResult Signout()
    {
        UserRetrieveService.RemoveCachedUser(Cache, this.User.GetIdentifier().TryParseID32(), this.User.Identity.Name);
        HttpContext.SignOutAsync(Request.Scheme);
        HttpContext.SignOutAsync(CookieAuthenticationDefaults.AuthenticationScheme);
        HttpContext.RequestServices.GetService<IElevationHandler>()?.DeleteToken();
        if (HttpContext.Request.Query.ContainsKey("winauth"))
        {
