...
public class TranslationRepository : BaseRepository
{
    ...
    public ListResponse<TranslationItem> List(TranslationListRequest request)
    {
        ...
        {
            // was:     var json = JSON.Parse<Dictionary<string, JToken>>(File.ReadAllText(textsFilePath));
            var json = JSON.Parse<Dictionary<string, string>>(File.ReadAllText(textsFilePath));
    ...
    public SaveResponse Update(TranslationUpdateRequest request, IServiceProvider services)
    {
        ...
        string json = JSON.StringifyIndented(result); //.StringifyIndented(result, indentation: 2);
